...
With M-i, helm-swoop on the buffer can be called during isearch. By
issuing M-i one more, the search will be performed throughout all
present buffers with =helm-multi-swoop-all=.

I prefer preserving the default keymap for C-s as it causes least
distortion to the window layout and is already powerful enough.
